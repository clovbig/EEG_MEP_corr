...
for sub_id in subjects:
    blocks_eeg = [i for i in os.listdir(os.path.join(data_dir, sub_id, 'EEG')) if os.path.isdir(os.path.join(
        data_dir, sub_id, 'EEG', i))]
    blocks_emg = [i for i in os.listdir(os.path.join(data_dir, sub_id, 'EEG')) if os.path.isdir(os.path.join(
        data_dir, sub_id, 'EMG', i))]
    blocks = set(blocks_eeg + blocks_emg)
    ...
    for idx_block, b in enumerate(blocks):
        ...
        for seq in ['A1', 'B1', 'A2', 'B2', 'A3', 'B3']:
            ...
            seq_epochs_emg = loadmat(os.path.join(data_dir, sub_id, 'EMG', b, 'Cleaned Data', f'TiMeS_{sub_id}_{b}_'
                                                  f'{seq}_GUI_Results.mat'))
            trial_pulses = seq_epochs_emg['States']
            for idx_t, t in enumerate(trial_pulses):
                print(f'trial n. {idx_t}')
                if t == 1:
                    sub_emg_data_sp.add_trial(seq_epochs_emg, idx_block, idx_t)
                elif t == 2:
                    sub_emg_data_dp.add_trial(seq_epochs_emg, idx_block, idx_t)
